...
export async function generateMetadata({ params }: { params: Promise<{ buyer: string }> }): Promise<Metadata> {
  ...
  const g = buyerGuides[buyer as BuyerSlug]?.es;
  if (!g) return {};
  return altMeta(`/guide/for/${buyer}`, "es", g.title, g.description);
}
...
export default async function Page({ params }: { params: Promise<{ buyer: string }> }) {
  ...
  const g = buyerGuides[buyer as BuyerSlug]?.es;
  if (!g) notFound();
  return <GuidePage guide={g} locale="es" path={`/guide/for/${buyer}`} />;
}
